...
def synchronize_gallery(folder: str, exif_updater: GalleryExifUpdate):
    ...
    def update_image_callback(image_name, image_title, image_description):
        exif_updater.update(images_path, image_name, image_title, image_description)
...
def synchronize_galleries_in_folder(
    folder: str, exif_updater: GalleryExifUpdate
) -> None:
...
def update_image_descriptions_in_folder(
    folder: str, exif_updater: GalleryExifUpdate
) -> None:
    if is_gallery_path(folder):
        ...
        if xml_gallery_tree is not None:
            ...
            print(
                f"Exception: parsing of {GALLERY_FILE_GALLERY} failed for \
                {folder}"
            )
    else:
        for folder_element in os.scandir(folder):
