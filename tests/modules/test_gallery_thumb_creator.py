...
from modules.gallery_configuration import (
    THUMB_QUALITY,
...
class TestGalleryThumbCreator(unittest.TestCase):
    ...
    def test_create_thumb(self, mock_image_enhance, mock_image):
        mock_image_object = self.__mock_image_image_class(
            mock_image, mock_image_enhance
        )
        ...
        mock_image.open.assert_called_once_with(
            path.join(self.__images_path, self.__thumb_name)
        )
        ...
        mock_image_object.enhance.assert_called_once_with(THUMB_SHARPEN_FACTOR)
        mock_image_object.save.assert_called_once_with(
            path.join(self.__thumbs_path, self.__thumb_name), quality=THUMB_QUALITY
        )
